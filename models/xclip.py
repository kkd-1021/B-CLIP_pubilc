...
import clip
from models.classification_net import Classification_net
class XCLIP(CLIP):
    def __init__(self,
                 embed_dim: int,
                 # vision
        ...
        self.classification_net=Classification_net(in_features=768,hidden_size=128)
    ...
    def encode_text(self, text):
        ...
        print("\033[0;31;40m encode text\033[0m")
        print(x.shape)
    ...
    def forward(self, image, text=None, val=True):
        ...
        if val==True:
            ...
            for i in range(video_features.shape[0]):
                cls = self.classification_net.forward(video_features[i].reshape(-1))
                cls_output.append(cls)
            cls_output = torch.stack(cls_output)

            print("\033[0;31;40mcls_output\033[0m")
            print(cls_output)
            return cls_output
